...
(function initMinaGPT() {
  ...
  // Persona: Mina's account manager. Every rule carries 2-3 phrasings and one
  // is picked at random, so asking twice never sounds canned. Facts confirmed
  // by Mina 2026-09-19: 750+ restaurants, 10 years, 8 A'Design Awards total
  // (2020-2024; 3 of them for Gong cha-era work), 14 exhibitions / 5 countries.
  ...
  const ANSWERS = [
    ...
    { id: "education", match: /education|degree|school|university|college|study|studied|mfa|master|bootcamp|certificat/i, a: [
    ...
    { id: "contact", match: /contact|email|hire|hiring|reach|linkedin|resume|cv/i, a: [
  ...
  const reply = (q) => {
    ...
    if (GUARD.test(q)) return { rule: "fallback", text: pick(fb) };
    const set = IS_ZH ? ANSWERS_ZH : ANSWERS;
    const hit = set.find((c) => c.match.test(q));
    return hit ? { rule: hit.id, text: pick(hit.a) } : { rule: "fallback", text: pick(fb) };
  };
  ...
  const ask = (q, via) => {
    ...
    if (typeof window.portfolioNotify === "function") {
      window.portfolioNotify("question", {
        ...
        answer: r.text, rule: r.rule, sid: chatSid,
      });
    }
    finishCurrent();
    openChat();
