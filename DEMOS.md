...
| `dashboard-08-weekly-report` | dashboard.html / zh | Solution 04「Weekly Report」的圖 | 檔案 Ng5Qec5PY0iDOczXTzjGc8,frame 5407:3522(6 模組 / 7 卡片的手機長捲動) | **兩支手機螢幕面板並排**(圓角+陰影,不做外殼 mockup — 直式形狀本身就讀作「手機」,如同橫式讀作「網站」),各自慢慢自動捲動(hover / 滑出視窗會暫停)。6 模組 3+3 分組:**左=業績面**(Trend for sales and upsell〔2 卡〕→ Top sales items → Sales channel distribution),**右=顧客面**(Member analysis → Review summary〔甜甜圈〕→ Off-premise geo〔簡化向量地圖〕)。圖表全手繪 SVG/CSS(分組長條、單系列柱、折線、甜甜圈、抽象地圖)。假資料 Golden Ember BBQ #A62、2026-07-04~10 一週,與其他圖同店。圖說不提數字(「你口袋裡的一週,一次滑完」)。fit() 有 0 寬保護 + window resize/load 監聽。**註:自動化分頁在背景時 rAF 暫停、截圖會空白,非 bug** |
